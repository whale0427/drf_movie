...
function resetPassword(){
  ...
  axios
      .post("/api/users/reset_password_confirm/",datas)
      .then(response=>{
        // showMessage("密码重置成功，即将跳转登录页面，请稍等","success",()=>{window.location.href="/login"},1500)
        showMessage("密码重置成功，即将跳转登录页面，请稍等","success",()=>{
...
<template>
  <div class="flex w-full h-full items-center justify-center tracking-wider">
    <div class="flex flex-col shadow-md shadow-[#B64529] bg-white rounded mt-35 w-[320px] h-[310px]">
      <div class="flex font-bold text-2xl my-10 justify-center">重置密码</div>
      <form method="post">
        <div class="flex items-center mb-8">
          <input v-model="new_password" class="absolute border border-gray-400 h-10 w-60 ml-10 pl-10 outline-none rounded focus:shadow-xs focus:shadow-[#B64529]" type="password" placeholder="请输入新密码">
        ...
        <div class="flex items-center mb-8">
          <input v-model="re_new_password" class="absolute border border-gray-400 h-10 w-60 ml-10 pl-10 outline-none rounded focus:shadow-xs focus:shadow-[#B64529]" type="password" placeholder="请再次输入新密码">
        ...
        <div class="flex items-center justify-center mb-1">
          <button v-on:click.prevent="resetPassword" class="h-10 w-60 bg-[#B64529] rounded text-white transition-all duration-200 hover:shadow-md hover:shadow-[#B64529]">
            提交
          </button>
        </div>
      </form>
